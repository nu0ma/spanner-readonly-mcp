...
Runtime env vars (required for `dev`/`start`): `SPANNER_PROJECT`, `SPANNER_INSTANCE`, `SPANNER_DATABASE`. Optional `SPANNER_EMULATOR_HOST` (e.g. `127.0.0.1:15000` for Spanner Omni, the recommended local backend) points the client at a local Spanner. The legacy `gcr.io/cloud-spanner-emulator/emulator` (port `9010`) also works but is no longer covered by tests.
...
Tests run against Spanner Omni rather than the legacy emulator because Omni matches production's read-only-snapshot DML rejection semantics; the emulator is more permissive.
